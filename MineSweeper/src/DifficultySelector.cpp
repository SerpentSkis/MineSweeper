...
DifficultySelector::DifficultySelector(float width, float height){
    ...
    menu[0].setPosition(sf::Vector2f(width/2.5, height/(MAX_NUMBER_OF_ITEMS_DIFF + 1) * 1));
    
    
    menu[1].setFont(font);
    menu[1].setColor(sf::Color::White);
    menu[1].setString("Medium");
    menu[1].setPosition(width/2.5, height/(MAX_NUMBER_OF_ITEMS_DIFF + 1) * 2);
    
    
    menu[2].setFont(font);
    menu[2].setColor(sf::Color::White);
    menu[2].setString("Hard");
    menu[2].setPosition(width/2.5, height/(MAX_NUMBER_OF_ITEMS_DIFF + 1) * 3);
    
    menu[3].setFont(font);
    menu[3].setColor(sf::Color::White);
    menu[3].setString("Back");
    menu[3].setPosition(width/2.5, height/(MAX_NUMBER_OF_ITEMS_DIFF + 1) * 4);
